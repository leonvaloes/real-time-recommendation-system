...
`UserAuth.CleanArch.Infra.Data` expects `MYSQL_CONNECTION_STRING`. `CatalogServiceMvc` expects `POSTGRES_CONNECTION_STRING`. Do not hardcode production credentials in source files; prefer local environment configuration and the Docker Compose stack for development.
