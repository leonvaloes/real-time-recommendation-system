...
`UserAuth.CleanArch.Infra.Data` expects `MONGODB_URI` and `MONGODB_DB` environment variables. Do not hardcode credentials in source files; prefer local environment configuration and the Docker Compose stack for development.
